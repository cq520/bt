...
{	
  ...
  pip install paramiko==2.7.1
	...
	if [ "$?" -ne 0 ];then
		wget -O /www/server/panel/plugin/ftp/ftp_main.py $download_Url/install/plugin/ftp/ftp_main2.py -T 5
		wget -O /www/server/panel/plugin/ftp/index.html $download_Url/install/plugin/ftp/index2.html -T 5
		wget -O /www/server/panel/plugin/ftp/info.json $download_Url/install/plugin/ftp/info2.json -T 5
